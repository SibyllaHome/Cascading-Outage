...
function [LS,network_after] = Dynamic_Load_Shedding_Calculator(faulty_lines, faulty_nodes, target_load, amount_of_shedding)
    ...
    network = case39;
    network.bus(:, 3:4) = network.bus(:, 3:4) * 1.0; % Pd, Qd
    load_initial = sum(network.bus(:, 3)); %sum up Pd
    network1 = network;
    network1.branch(faulty_lines, 11) = 0; % set the faulty lines' status to 0
%     network2 = trip_nodes(network1, faulty_nodes);
    network1.gen(ismember(network1.gen(:, 1), faulty_nodes), :) = 0; %set every elements of faulty nodes o 0
    network2 = network1;
    network3 = network2;
...
%             network3 = trip_nodes(network3, island_nodes);
            network3.gen(ismember(network3.gen(:, 1), island_nodes), :) = 0;
...
%             network3 = trip_nodes(network3, island_nodes);
    ...
    LS = [load_shedding, LS_balance, LS_tripped_gen, LS_tripped_load, LS_unsupplied]
end
